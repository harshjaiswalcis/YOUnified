  ...
  Widget build(BuildContext context) {
    return Consumer<GrievanceProvider>(
      builder: (context, provider, _) {
        ...
        final List<Grievance> filteredGrievances =
            provider.grievances.where((g) {
          if (isOpen) {
            return g.status.toLowerCase() != 'closed';
          } else {
            return g.status.toLowerCase() == 'closed';
          }
        }).toList();
        ...
        if (filteredGrievances.isEmpty) {
          return Center(
            child: Text(
              isOpen
                  ? "No ${context.strings.open} ${context.strings.grievances.toLowerCase()}"
                  : "No ${context.strings.closed} ${context.strings.grievances.toLowerCase()}",
              style: context.textTheme.bodyMedium,
            ),
          );
        }
