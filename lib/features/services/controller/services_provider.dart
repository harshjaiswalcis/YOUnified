...
class ServicesProvider extends ChangeNotifier {
  ...
  //MAPPING
  static final List<HomeListModel> servicesListData =
      servicesList.map((mapData) => HomeListModel.fromMap(mapData)).toList();
  ...
  //MAPPING
  static final List<VotingModel> votingListData =
      votingList.map((mapData) => VotingModel.fromJson(mapData)).toList();
...
//MAPPING
  ...
  String formatDateString(String dateString) {
    ...
    String daySuffix = getDaySuffix(
        parsedDate.day); // Get the day suffix (e.g., "th", "st", etc.)
    String formattedDate =
        "${DateFormat("MMMM d").format(parsedDate)}$daySuffix ${DateFormat("y").format(parsedDate)}";
...
// Function to get the correct suffix for the day (st, nd, rd, th)
  ...
  List<String> selectedColorCombo = [];
  ...
  void toggleColorCombo(String color) {
    if (selectedColorCombo.contains(color)) {
      selectedColorCombo.remove(color);
    } else {
      selectedColorCombo.add(color);
    }
    notifyListeners();
  }
  ...
  void resetSelections() {
    selectedColor = null;
    selectedSeason = null;
    selectedColorCombo.clear();
    notifyListeners();
  }

  bool get canProceedFromFirst => selectedColor != null;
  bool get canProceedFromSecond => selectedSeason != null;
  bool get canProceedFromThird => selectedColorCombo.isNotEmpty;
}
